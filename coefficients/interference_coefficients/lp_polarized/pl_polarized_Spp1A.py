...
def calculate_s_1_plus_plus_longitudinally_polarized_A(
    ...
    try:
        ...
        almost_prefactor = Decimal("8. ") * target_polarization * shorthand_k / root_one_plus_epsilon_squared**6

        # (7): Calculate the first bracket term:
        first_bracket_term = root_one_plus_epsilon_squared - Decimal("1.") + (t_over_Q_squared * (Decimal("1.") + root_one_plus_epsilon_squared - Decimal("2.") * x_Bjorken))

        # (8): Calculate the second bracket term:
        second_bracket_term = Decimal("1.") - (t_over_Q_squared * (Decimal("3.")  - root_one_plus_epsilon_squared - 6. * x_Bjorken) / three_plus_root_epsilon_stuff)

        # (9): Calculate the first prefactor:
        prefactor_one = Decimal("-1.0") * almost_prefactor * (Decimal("2.") - Decimal("2.") * lepton_energy_fraction_y + lepton_energy_fraction_y**2 + Decimal("2.") * epsilon_y_over_2_squared) * xB_t_over_Q_squared

        # (10): Calculate the second prefactor:
        prefactor_two = almost_prefactor * (Decimal("1.") - lepton_energy_fraction_y - epsilon_y_over_2_squared) * three_plus_root_epsilon_stuff * xB_t_over_Q_squared
